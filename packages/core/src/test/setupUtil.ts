 ...
 *   pre-seeded from outside the extension — see brain/references/amazon-q-vscode-e2e-auth.md).
 ...
 * make their assertions while this hook is still active (i.e. inside the `using()` block).
 *
...
export function registerStaticBearerToken(token = process.env['BEARER_TOKEN']): vscode.Disposable {
    ...
    const createTokenStub = patchObject(
        SsoAccessTokenProvider.prototype,
        'createToken',
        async () => ssoToken as any
    )
